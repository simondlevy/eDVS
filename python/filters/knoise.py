...
class Knoise:
    ...
    def _initialize_last_times_map_for_noise_rate(self, last_timestamp_us):
        '''
    ...
    def _initialize_row_or_col(self, ts, x_or_y, s, last_timestamp_us):

        for i in range(len(ts)):

            p = np.random.random()
            t = -self.noise_rate_hz * np.log(1 - p)
            tUs = int((1000000 * t))
